...
import type { WorkflowV17InvocationSnapshot, WorkflowV17MeasurementResourceBinding } from "../persistence/workflow-v17-invocation.js";
import {
...
import type { WorkflowV17StaticEffectResources } from "../runtime/effect-adapters-v17.js";
...
import { WorkflowV17MetricSetRuntime, type WorkflowV17NormalizedMetricSet } from "./metric-set-v17.js";

const MAX_REVISION_RETRIES = 16;

export interface WorkflowV17MeasurementLaunchWorkspace {
  root: string;
  cwd: string;
  treeHash: string;
}
...
interface ResolvedMeasurement {
  ...
  metricSet: ReturnType<WorkflowV17MetricSetRuntime["metricSet"]>;
  normalized: WorkflowV17NormalizedMetricSet;
...
export interface WorkflowV17MeasurementAdapterOptions {
...
export class WorkflowV17MeasurementEffectAdapter implements WorkflowV17SemanticEffectAdapter {
  readonly kind = "measure" as const;
  private readonly now: () => Date;
  private readonly resolved = new Map<string, ResolvedMeasurement>();

  constructor(private readonly options: WorkflowV17MeasurementAdapterOptions) {
  ...
  async journalIdentity(
    context: WorkflowV17EffectAdapterContext & { operation: WorkflowOperationV17Record },
  ): Promise<WorkflowV17EffectIdentity> {
    const resolved = this.resolve(context.input);
  ...
  async execute(
    ...
    try {
      ...
      const samples: WorkflowMeasurementSampleV17Record[] = [];
  ...
  evidence(context: WorkflowV17EffectAdapterContext & { result: JsonValue }): { artifacts: WorkflowCallArtifactV17Input[] } {
  ...
  ): Promise<WorkflowMeasurementV17Record> {
  ...
  private requireResolved(operation: WorkflowOperationV17Record, input: unknown): ResolvedMeasurement {
  ...
  private resolve(value: unknown): ResolvedMeasurement {
    ...
    let candidate: WorkflowCandidateV17Record | undefined;
...
export class WorkflowV17ExperimentEffectAdapter implements WorkflowV17SemanticEffectAdapter {
  ...
  async restore(context: WorkflowV17EffectRestoreContext): Promise<JsonObject> {
    const result = parseExperiment(context.result);
...
function resolveProfile(
  ...
  operationSite: string,
  selector: string,
  metrics: WorkflowV17NormalizedMetricSet,
...
function assertDynamicUse(
  ...
  use: WorkflowV17MeasurementResourceBinding["uses"][number],
  metrics: WorkflowV17NormalizedMetricSet,
): void {
...
function storedMeasurement(
  record: WorkflowMeasurementV17Record,
  authorityId?: string,
  metricSet?: Pick<ReturnType<WorkflowV17MetricSetRuntime["metricSet"]>, "policyHash" | "samplingHash">,

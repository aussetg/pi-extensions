import type { WorkflowV17RunProjection } from "./types-v17.js";
...
export function renderWorkflowV17RunProjection(projection: WorkflowV17RunProjection): string[] {
  const lines = [
...
export function renderWorkflowV17RunProjectionText(projection: WorkflowV17RunProjection): string {
  return `${renderWorkflowV17RunProjection(projection).join("\n")}\n`;
}

function sumCounts(counts: WorkflowV17RunProjection["operationCounts"]): number {
...
function safe(value: unknown, maximum: number = 512): string { return boundedWorkflowV17ProjectionText(value, maximum); }

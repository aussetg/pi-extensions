export const WORKFLOW_TOOL_DESCRIPTION = "Run a constrained JavaScript workflow that orchestrates multiple Pi subagents. Use only for complex or non-standard orchestration that a single sequential agent cannot handle: explicit workflow requests, fan-out, adversarial verification, broad audits, large migrations, or staged repeatable procedures. Do not use for work that can be done sequentially with normal tools or by one agent.";
...
export const WORKFLOW_PROMPT_GUIDELINES = [
  ...
  "Good workflow cases: explicit workflow requests, fan-out to multiple subagents, adversarial cross-checking, broad repository audits, large migrations with independent phases, and repeatable staged procedures.",
  ...
  "Workflow control scripts are constrained: do not use globalThis, Date.now(), Math.random(), argless new Date(), imports, require, process, fs, fetch, or network APIs.",
  "The parser and child VM membrane enforce the workflow API; bwrap/systemd-run contain the control child at the OS boundary, and launch fails rather than falling back unsandboxed if those host tools are unavailable.",
  ...
  "agent(prompt, opts) accepts only strict JSON options: label, phase, schema, model, thinking, workspace, and stallMs. workspace is 'shared', 'readOnly', or 'patch'. Unknown keys or invalid values fail before launch.",
  ...
  "Workflow progress is rendered automatically from phases, agent calls created by loops/branches/fan-out, call state, usage, and logs. Workflow scripts do not define UI.",

...
import { collectWorkflowV17SchemaResources } from "../definition/workflow-v17-schema.js";
import {
  WORKFLOW_V17_RUNTIME_API_DESCRIPTOR,
  WORKFLOW_V17_RUNTIME_API_HASH,
  WORKFLOW_V17_RUNTIME_API_VERSION,
  type WorkflowV17ResourceIdentity,
...
import {
  ...
  type WorkflowV17Id,
} from "../registry/structured-workflows-v17.js";
import type {
  WorkflowV17Exposure,
  WorkflowV17RegistryPolicySnapshot,
} from "../registry/workflow-v17-policy.js";
import { workflowV17Exposure } from "../registry/workflow-v17-policy.js";

export type WorkflowV17LaunchAuthority = "model" | "user" | "rpc";

export interface WorkflowV17MetricOutputBinding {
  output: string;
  role: "primary" | "guardrail" | "observe";
}

export interface WorkflowV17MeasurementResourceUse {
  operationSite: string;
  metricPolicyPath: string;
  samplingPath?: string;
  policy: JsonObject;
  sampling?: JsonObject;
  outputs: WorkflowV17MetricOutputBinding[];
}

export interface WorkflowV17MeasurementResourceBinding {
  formatVersion: 1;
  identity: WorkflowV17ResourceIdentity & { kind: "measurement-profile" };
  inputPath: string;
  profile: MeasurementProfileSnapshot;
  uses: WorkflowV17MeasurementResourceUse[];
  bindingHash: string;
}

export interface PersistedWorkflowV17Invocation {
  formatVersion: 1;
  ...
  launch: {
    authority: WorkflowV17LaunchAuthority;
    policyHash: string;
...
export interface WorkflowV17InvocationSnapshot extends PersistedWorkflowV17Invocation {
  source: string;
  executableSource: string;
  policy: WorkflowV17RegistryPolicySnapshot;
  resources: WorkflowV17MeasurementResourceBinding[];
}

export interface CreateWorkflowV17InvocationOptions {
  authority: WorkflowV17LaunchAuthority;
  projectTrusted?: boolean;
  measurementProfiles?: { resolve(selector: string): MeasurementProfileSnapshot };
}

export interface WorkflowV17InvocationFilesystemPaths {
  root: string;
  source: string;
  executable: string;
  context: string;
...
export function createWorkflowV17InvocationSnapshot(
  ref: WorkflowV17DefinitionRef,
  args: unknown,
  options: CreateWorkflowV17InvocationOptions,
): WorkflowV17InvocationSnapshot {
  ...
  const snapshot: WorkflowV17InvocationSnapshot = {
    ...persisted,
  ...
  return deepFreezeJson(snapshot as unknown as JsonValue) as unknown as WorkflowV17InvocationSnapshot;
}

export function workflowV17InvocationFilesystemPaths(
  runDirInput: string,
): WorkflowV17InvocationFilesystemPaths {
...
export async function writeWorkflowV17InvocationSnapshot(
  runDirInput: string,
  snapshot: WorkflowV17InvocationSnapshot,
): Promise<void> {
  assertWorkflowV17InvocationSnapshot(snapshot);
  validateSnapshotAgainstSource(snapshot);
  const paths = workflowV17InvocationFilesystemPaths(runDirInput);
  ...
  const temporaryPaths = workflowV17InvocationFilesystemPaths(temporary);
  ...
  try {
    await Promise.all([
      ...
      writeExclusive(temporaryPaths.runtimeApi, `${stableJson({
        version: WORKFLOW_V17_RUNTIME_API_VERSION,
        hash: WORKFLOW_V17_RUNTIME_API_HASH,
        descriptor: WORKFLOW_V17_RUNTIME_API_DESCRIPTOR,
      })}\n`),
    ]);
...
export async function readWorkflowV17InvocationSnapshot(
  runDirInput: string,
): Promise<WorkflowV17InvocationSnapshot> {
  const paths = workflowV17InvocationFilesystemPaths(runDirInput);
  ...
  const [source, executableSource, persisted, policy, resources, runtimeApi] = await Promise.all([
    ...
    readCanonicalJson<PersistedWorkflowV17Invocation>(paths.invocation, SNAPSHOT_JSON_BYTES),
    readCanonicalJson<WorkflowV17RegistryPolicySnapshot>(paths.policy, SNAPSHOT_JSON_BYTES),
    readCanonicalJson<WorkflowV17MeasurementResourceBinding[]>(paths.resources, SNAPSHOT_JSON_BYTES),
  ...
  const snapshot = {
    ...
    policy,
    resources,
  } as WorkflowV17InvocationSnapshot;
  assertWorkflowV17InvocationSnapshot(snapshot);
  validateSnapshotAgainstSource(snapshot);
  return deepFreezeJson(snapshot as unknown as JsonValue) as unknown as WorkflowV17InvocationSnapshot;
}

export function assertWorkflowV17InvocationSnapshot(
  snapshot: WorkflowV17InvocationSnapshot,
): void {
  if (snapshot.formatVersion !== 1 || snapshot.runtimeApiVersion !== WORKFLOW_V17_RUNTIME_API_VERSION
    || snapshot.runtimeApiHash !== WORKFLOW_V17_RUNTIME_API_HASH) {
  ...
  if (sha256(snapshot.source) !== snapshot.sourceHash
    ...
    || snapshot.transform.runtimeApiHash !== WORKFLOW_V17_RUNTIME_API_HASH) {
  ...
  if (workflowV17Exposure(snapshot.policy, snapshot.name) !== snapshot.exposure) {
...
function persistedInvocationBody(
  ...
  authority: WorkflowV17LaunchAuthority,
  projectTrusted: boolean,
): Omit<PersistedWorkflowV17Invocation, "snapshotHash"> {
  return {
    formatVersion: 1,
    workflowId: ref.id,
...
function persistedFromSnapshot(snapshot: WorkflowV17InvocationSnapshot): PersistedWorkflowV17Invocation {
  const {
    source: _source,
...
function prepareInvocationResources(
  ...
  input: JsonObject,
  profiles: CreateWorkflowV17InvocationOptions["measurementProfiles"],
  projectTrusted: boolean,
): WorkflowV17MeasurementResourceBinding[] {
  const declared = collectWorkflowV17SchemaResources(parsed.metadata.input);
  if (declared.length === 0) return [];
  if (!profiles) throw new Error("Invocation-selected measurement profiles require a trusted registry");
  const groups = new Map<string, WorkflowV17DynamicResourceUse[]>();
  ...
  return [...groups.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([inputPath, uses]) => {
    ...
    const identity: WorkflowV17ResourceIdentity & { kind: "measurement-profile" } = {
      formatVersion: 1,
...
function metricOutputBindings(
  ...
  profile: MeasurementProfileSnapshot,
  inputPath: string,
): WorkflowV17MetricOutputBinding[] {
  ...
  const result: WorkflowV17MetricOutputBinding[] = [metricOutput(primary, "primary", profile)];
...
function metricOutput(
  ...
  role: WorkflowV17MetricOutputBinding["role"],
  profile: MeasurementProfileSnapshot,
): WorkflowV17MetricOutputBinding {
...
function assertResourceBindings(resources: readonly WorkflowV17MeasurementResourceBinding[]): void {
...
function assertPolicy(
  policy: WorkflowV17RegistryPolicySnapshot,
  namespace: PersistedWorkflowV17Invocation["namespace"],
  expectedHash: string,
): void {
  exactKeys(
...
function assertSnapshotFrontend(
  ...
  if (parsed.installedName !== snapshot.name || parsed.sourceHash !== snapshot.sourceHash
    ...
    || workflowV17DefinitionHash(snapshot.workflowId, parsed) !== snapshot.definitionHash
...
function validateSnapshotAgainstSource(snapshot: WorkflowV17InvocationSnapshot): void {
  const parsed = parseWorkflowV17(snapshot.source, { fileName: `${snapshot.name}.flow.ts` });
...
function resourceResolver(resources: readonly WorkflowV17MeasurementResourceBinding[]) {
...
function persistedInvocationKeys(value: PersistedWorkflowV17Invocation): string[] {
  return [
...
function persistedShape(value: unknown): asserts value is PersistedWorkflowV17Invocation {
  const record = plainRecord(value, "Workflow v17 persisted invocation");
  const expected = persistedInvocationKeys(record as unknown as PersistedWorkflowV17Invocation).sort();
...
function assertRuntimeApi(value: unknown): void {
  ...
  if (keys.join(",") !== "descriptor,hash,version"
    || record.version !== WORKFLOW_V17_RUNTIME_API_VERSION
    || record.hash !== WORKFLOW_V17_RUNTIME_API_HASH
    || stableHash(record.descriptor) !== WORKFLOW_V17_RUNTIME_API_HASH) {

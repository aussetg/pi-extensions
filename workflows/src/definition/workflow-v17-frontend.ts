...
import { analyzeWorkflowV17Source } from "./workflow-v17-analysis.js";
import {
  collectWorkflowV17SchemaResources,
  WorkflowV17SchemaEvaluator,
} from "./workflow-v17-schema.js";
import {
  typecheckWorkflowV17Source,
  validateWorkflowV17TypeScriptEnvelope,
} from "./workflow-v17-typecheck.js";
import type {
  ParsedWorkflowV17,
...
export interface ParseWorkflowV17Options {
  fileName?: string;
  apiPath?: string;
}
...
export function parseWorkflowV17(
  source: string,
  options: ParseWorkflowV17Options = {},
): ParsedWorkflowV17 {
  ...
  validateWorkflowV17TypeScriptEnvelope(source, fileName);
  ...
  const definition = findV17Definition(ast, imports);
  ...
  typecheckWorkflowV17Source(source, {
  ...
  const evaluator = new WorkflowV17SchemaEvaluator(schemaBinding, constants.initializers);
  ...
  const analysis = analyzeWorkflowV17Source({
    ast,
  ...
  const { executableSource, transform } = buildV17ExecutableSource({
    source,
    strippedSource,
    imports,
    definition,
  ...
  const parsed: ParsedWorkflowV17 = {
    formatVersion: 1,
    fileName: path.basename(fileName),
    installedName: path.basename(fileName, WORKFLOW_V17_SOURCE_EXTENSION),
    source,
  ...
  return deepFreezeJson(parsed as unknown as JsonValue) as unknown as ParsedWorkflowV17;
}

function validateFileName(fileNameInput: string): string {
  const fileName = path.basename(fileNameInput);
  if (!fileName.endsWith(WORKFLOW_V17_SOURCE_EXTENSION)) {
    throw new WorkflowScriptError(`Workflow source must use ${WORKFLOW_V17_SOURCE_EXTENSION}`);
  }
  const stem = fileName.slice(0, -WORKFLOW_V17_SOURCE_EXTENSION.length);
...
function parseVirtualImport(ast: WorkflowAstNode): VirtualImportBindings {
  ...
  if (imports.length !== 1) {
    throw new WorkflowScriptError(
      `A workflow requires exactly one import from ${JSON.stringify(WORKFLOW_V17_MODULE)}`,
      location(imports[1] ?? imports[0] ?? ast),
    );
  }
  const declaration = imports[0]!;
  if (declaration.source?.value !== WORKFLOW_V17_MODULE) {
    throw new WorkflowScriptError(
      `Workflow imports are restricted to ${JSON.stringify(WORKFLOW_V17_MODULE)}`,
...
function findV17Definition(ast: WorkflowAstNode, imports: VirtualImportBindings): WorkflowDefinitionAst {
...
function validateStaticTopLevelBindings(
  ...
  evaluator: WorkflowV17SchemaEvaluator,
): void {
...
function parseAgentDescriptor(
  ...
  const identity: WorkflowV17DescriptorIdentity = {
    formatVersion: 1,
    kind: "agent-task",
    sourceSite,
...
function buildV17ExecutableSource(input: {
  source: string;
...
}): { executableSource: string; transform: WorkflowV17SourceTransform } {
  ...
  const transform: WorkflowV17SourceTransform = {
    ...body,
    operationSites: body.operationSites as WorkflowV17SourceTransform["operationSites"],
...
function rejectExecutableResources(schema: JsonSchema, node: WorkflowAstNode, label: string): void {
  if (collectWorkflowV17SchemaResources(schema).length) {

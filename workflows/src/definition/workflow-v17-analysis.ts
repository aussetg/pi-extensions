...
import {
  collectWorkflowV17SchemaResources,
  isWorkflowV17MeasurementProfileSchema,
  isWorkflowV17SafePathSchema,
  workflowV17SchemaAtPath,
  type WorkflowV17SchemaEvaluator,
} from "./workflow-v17-schema.js";
import type {
  ...
  WorkflowV17ExecutionContext,
  WorkflowV17HelperSummary,
  WorkflowV17NativeLoop,
  WorkflowV17OperationSite,
  WorkflowV17Review,
} from "./workflow-v17-types.js";

export const WORKFLOW_V17_FLOW_METHODS = Object.freeze([
  "parallel",
  "map",
  "agent",
  "command",
...
type FlowMethod = WorkflowV17OperationSite["method"];
...
interface InternalOperationSite extends WorkflowV17OperationSite {
  node: WorkflowAstNode;
  owner: WorkflowAstNode;
}

export interface AnalyzeWorkflowV17SourceInput {
  ast: WorkflowAstNode;
  ...
  schemaEvaluator: WorkflowV17SchemaEvaluator;
  descriptors: readonly WorkflowV17Descriptor[];
  parents: Map<WorkflowAstNode, { parent: WorkflowAstNode; key: string }>;
}

export interface AnalyzeWorkflowV17SourceResult {
  ...
  review: Omit<WorkflowV17Review, "maximumConcurrency">;
}

const FLOW_METHOD_SET = new Set<string>(WORKFLOW_V17_FLOW_METHODS);
...
const CONTEXT_ORDER: WorkflowV17ExecutionContext[] = ["root", "concurrent", "candidate", "key"];

export function analyzeWorkflowV17Source(
  input: AnalyzeWorkflowV17SourceInput,
): AnalyzeWorkflowV17SourceResult {
  ...
  const dynamicResources: WorkflowV17DynamicResourceUse[] = [];
  const candidateWrites: WorkflowV17CandidateWriteSite[] = [];
  ...
  const helpers = model.functions
    ...
    .map((fn): WorkflowV17HelperSummary => ({
  ...
  const review: Omit<WorkflowV17Review, "maximumConcurrency"> = {
    capabilities: [...capabilities].sort(),
    agentProfiles: uniqueSorted(input.descriptors
      .filter((descriptor): descriptor is WorkflowV17AgentDescriptor => descriptor.kind === "agent-task")
      .map((descriptor) => descriptor.profile)),
    commandProfiles: uniqueSorted(input.descriptors
      .filter((descriptor): descriptor is WorkflowV17CommandDescriptor => descriptor.kind === "command-task")
      .map((descriptor) => descriptor.profile)),
...
function validateOperationSite(state: ValidateOperationInput): void {
  ...
  switch (site.method) {
    ...
    case "ask": {
      ...
      if (collectWorkflowV17SchemaResources(responseSchema).length) {
    ...
    case "measure": {
      ...
      if (profile?.type === "Literal" && typeof profile.value === "string") {
        ...
        const schema = inputPath ? workflowV17SchemaAtPath(input.inputSchema, inputPath) : undefined;
        if (!inputPath || !isWorkflowV17MeasurementProfileSchema(schema)) {
          throw new WorkflowScriptError(
...
function validateSiteConcurrency(input: AnalyzeWorkflowV17SourceInput, site: InternalOperationSite): void {
...
function candidateWriteSite(
  ...
  if (inputPath) {
    const schema = workflowV17SchemaAtPath(state.input.inputSchema, inputPath);
...
function isSafeWriteSchema(schema: JsonSchema | undefined): boolean {
  ...
  if (schema.type === "array") return isWorkflowV17SafePathSchema(schema.items as JsonSchema | undefined);
  ...
  return Boolean(
    allow?.type === "array" && isWorkflowV17SafePathSchema(allow.items as JsonSchema | undefined)
    && (!deny || (deny.type === "array" && isWorkflowV17SafePathSchema(deny.items as JsonSchema | undefined))),
  );
}
...
function isConstVariableInitializer(input: AnalyzeWorkflowV17SourceInput, node: WorkflowAstNode): boolean {
...
function buildFunctionModel(input: AnalyzeWorkflowV17SourceInput): FunctionModel {
...
function collectNativeLoops(
  ...
  const result: Array<WorkflowV17NativeLoop & { start: number }> = [];
...
function loopKind(node: WorkflowAstNode): WorkflowV17NativeLoop["kind"] | undefined {
...
function loopBound(
  input: AnalyzeWorkflowV17SourceInput,
  node: WorkflowAstNode,
  kind: WorkflowV17NativeLoop["kind"],
): WorkflowV17NativeLoop["bound"] {
  ...
  let result: WorkflowV17NativeLoop["bound"] = "unknown";
  walk(node.test, (candidate) => {
    ...
    walk(bound, (part) => {
      ...
      const inputSchema = inputPath ? workflowV17SchemaAtPath(input.inputSchema, inputPath) : undefined;
...
function deriveCapabilities(
  ...
  dynamicResources: readonly WorkflowV17DynamicResourceUse[],
): Set<WorkflowV17Capability> {
  const result = new Set<WorkflowV17Capability>();
...
function validateParallelCall(call: WorkflowAstNode): Pick<WorkflowV17OperationSite, "parallelKeys" | "errors" | "requestedConcurrency"> {
...
function validateMapCall(call: WorkflowAstNode): Pick<WorkflowV17OperationSite, "errors" | "requestedConcurrency"> {
...
): Pick<WorkflowV17OperationSite, "errors" | "requestedConcurrency"> {
...
function isAwaitedOrReturned(input: AnalyzeWorkflowV17SourceInput, node: WorkflowAstNode): boolean {
...
function summarizeContexts(signatures: readonly ContextSignature[]): WorkflowV17ExecutionContext[] {
  const values = new Set<WorkflowV17ExecutionContext>();
  for (const signature of signatures) {
...
function compareDynamicResources(left: WorkflowV17DynamicResourceUse, right: WorkflowV17DynamicResourceUse): number {

...
import { WORKFLOW_V17_MODULE } from "./workflow-language-v17.js";

const DEFAULT_API_PATH = fileURLToPath(new URL("../../workflow-api.d.ts", import.meta.url));

export interface WorkflowV17TypecheckOptions {
  fileName: string;
  apiPath?: string;
}

/** Inspect syntax that Node's strip-only pass deliberately erases, especially type-only imports. */
export function validateWorkflowV17TypeScriptEnvelope(source: string, fileName: string): void {
  ...
  if (!ts.isStringLiteral(declaration.moduleSpecifier)
    || declaration.moduleSpecifier.text !== WORKFLOW_V17_MODULE) {
    throw nodeError(
      file,
...
export function typecheckWorkflowV17Source(
  source: string,
  options: WorkflowV17TypecheckOptions,
): void {

...
import type { WorkflowV17SchemaResource } from "./workflow-v17-types.js";

export const WORKFLOW_V17_RESOURCE_KEY = "x-pi-workflow-resource";
export const WORKFLOW_V17_SAFE_PATH_KEY = "x-pi-workflow-safe-path";

export interface WorkflowV17StaticSchema {
...
type StaticValue =
  ...
  | WorkflowV17StaticSchema;

export class WorkflowV17SchemaEvaluator {
  ...
  schemaValue(node: WorkflowAstNode, label: string): WorkflowV17StaticSchema {
  ...
  private evaluateSchema(node: WorkflowAstNode, label: string): WorkflowV17StaticSchema {
    const value = this.evaluate(node, label);
  ...
  private evaluateSchemaCall(call: WorkflowAstNode, label: string): WorkflowV17StaticSchema {
    const method = call.callee.property.name as string;
    const args = call.arguments ?? [];
    const schema = (value: unknown, optional = false): WorkflowV17StaticSchema => ({
      kind: "schema",
    ...
    switch (method) {
      ...
      case "safePath":
        ...
        return schema({
          ...
          pattern: "^(?!/)(?!.*(?:^|/)\\.\\.?(?:/|$))(?!.*//)(?!.*\\\\)[^\\u0000-\\u001f\\u007f]+$",
          [WORKFLOW_V17_SAFE_PATH_KEY]: true,
        });
      case "json":
      ...
      case "measurementProfile":
        ...
        return schema({
          ...
          [WORKFLOW_V17_RESOURCE_KEY]: "measurement-profile",
        });
      case "raw": {
...
export function collectWorkflowV17SchemaResources(schema: JsonSchema): WorkflowV17SchemaResource[] {
  const result: WorkflowV17SchemaResource[] = [];
...
export function workflowV17SchemaAtPath(schema: JsonSchema, inputPath: string): JsonSchema | undefined {
...
export function isWorkflowV17MeasurementProfileSchema(schema: JsonSchema | undefined): boolean {
  return schema?.[WORKFLOW_V17_RESOURCE_KEY] === "measurement-profile";
}

export function isWorkflowV17SafePathSchema(schema: JsonSchema | undefined): boolean {
  return schema?.[WORKFLOW_V17_SAFE_PATH_KEY] === true;
}
...
function rejectOptional(value: WorkflowV17StaticSchema, node: WorkflowAstNode, label: string): void {

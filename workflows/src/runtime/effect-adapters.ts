...
import { materializeWorkflowAgentInputs } from "../artifacts/agent-inputs.js";
...
export interface WorkflowStaticEffectResources {
  ...
  verifications: Record<string, WorkflowStaticEffectBinding>;
...
export function workflowStaticEffectResources(input: {
  ...
  return deepFreezeJson({ ...body, hash: stableHash(body) } as unknown as JsonValue) as unknown as WorkflowStaticEffectResources;
}

export function assertWorkflowStaticEffectResources(
  workflow: ParsedWorkflow,
...
export interface WorkflowAgentExecutionRequest {
  ...
  binding: WorkflowStaticEffectBinding;
  prompt: string;
  artifacts: WorkflowArtifactManifest;
  inputs: AgentInputBundleHandle;
  workspace?: WorkflowCandidateWorkspaceHandle;
  signal: AbortSignal;
}
...
export interface WorkflowAgentExecutionResult {
  ...
  published?: WorkflowArtifactRecord[];
  usage?: JsonObject;
  resources?: JsonObject;
}

export interface WorkflowAgentEffectExecutor {
  execute(request: WorkflowAgentExecutionRequest): Promise<WorkflowAgentExecutionResult>;
}
...
export interface WorkflowCommandEffectExecutor {
  execute(request: WorkflowCommandExecutionRequest): Promise<WorkflowCommandExecutionResult>;
}

export interface WorkflowAskExecutor {
  ask(request: {
    run: WorkflowRunRecord;
    operation: WorkflowOperationRecord;
    prompt: string;
...
export interface WorkflowVerificationExecutor {
  verify(request: {
    run: WorkflowRunRecord;
    operation: WorkflowOperationRecord;
    attempt: WorkflowAttemptRecord;
...
export interface WorkflowApplyExecutor {
  apply(request: {
    run: WorkflowRunRecord;
    operation: WorkflowOperationRecord;
    attempt: WorkflowAttemptRecord;
    candidate: WorkflowCandidateRecord;
    workspace: WorkflowCandidateWorkspaceHandle;
    verification: WorkflowCandidateVerificationRecord;
    signal: AbortSignal;
  }): Promise<{
    receiptId: string;
    approvalId: string;
...
export interface WorkflowMeasurementAuthorityResolver {
...
export class WorkflowAgentEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "agent" as const;
  private readonly now: () => Date;
  private readonly resolved = new Map<string, ReturnType<WorkflowAgentEffectAdapter["resolve"]>>();

  constructor(private readonly options: AdapterOptions & { executor: WorkflowAgentEffectExecutor }) {
  ...
  async journalIdentity(context: WorkflowEffectAdapterContext & { operation: WorkflowOperationRecord }): Promise<WorkflowEffectIdentity> {
  ...
  async execute(context: WorkflowEffectAdapterContext & { operation: WorkflowOperationRecord }): Promise<JsonValue> {
    ...
    try {
      const workspace = resolved.workspace
        ...
        : undefined;
      const inputs = await materializeWorkflowAgentInputs({
  ...
  evidence(context: WorkflowEffectAdapterContext & { operation: WorkflowOperationRecord; result: JsonValue }) {
...
export class WorkflowCommandEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "command" as const;
  private readonly now: () => Date;
  private readonly resolved = new Map<string, ReturnType<WorkflowCommandEffectAdapter["resolve"]>>();

  constructor(private readonly options: AdapterOptions & { executor: WorkflowCommandEffectExecutor }) {
    this.now = options.now ?? (() => new Date());
  }
...
export class WorkflowAskEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "ask" as const;
  constructor(private readonly options: AdapterOptions & { executor: WorkflowAskExecutor }) {}
...
export class WorkflowVerificationEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "verify" as const;
  private readonly now: () => Date;
  constructor(private readonly options: AdapterOptions & { executor: WorkflowVerificationExecutor }) {
    this.now = options.now ?? (() => new Date());
  }
...
export class WorkflowAcceptEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "accept" as const;
  constructor(private readonly options: AdapterOptions & { measurements?: WorkflowMeasurementAuthorityResolver }) {}
  semanticInput(context: Omit<WorkflowEffectAdapterContext, "semanticInput" | "operation">): JsonValue {
    return this.resolve(context.input).semanticInput;
  }
  journalIdentity(context: WorkflowEffectAdapterContext & { operation: WorkflowOperationRecord }): WorkflowEffectIdentity {
  ...
  execute(context: WorkflowEffectAdapterContext & { operation: WorkflowOperationRecord }): JsonValue {
...
export class WorkflowRejectEffectAdapter implements WorkflowSemanticEffectAdapter {
...
export class WorkflowApplyEffectAdapter implements WorkflowSemanticEffectAdapter {
  readonly kind = "apply" as const;
  private readonly now: () => Date;
  constructor(private readonly options: AdapterOptions & { executor: WorkflowApplyExecutor }) {
    this.now = options.now ?? (() => new Date());
  }
  semanticInput(context: Omit<WorkflowEffectAdapterContext, "semanticInput" | "operation">): JsonValue {
    return this.resolve(context.input).semanticInput;
...
export function workflowVerificationRecord(
...
function verificationAuthority(
  ...
  const verification = workflowVerificationRecord(options, value);
...
function identity(
  ...
  run: WorkflowRunRecord,
  completionAuthority: WorkflowEffectIdentity["completionAuthority"],
  replayPolicy: WorkflowEffectIdentity["replayPolicy"],
): WorkflowEffectIdentity {
  return {
...
function workspaceAuthority(candidates: WorkflowCandidateRuntime, value: unknown): JsonObject {
...
function verificationAuthoritySemantic(value: WorkflowCandidateVerificationRecord): JsonObject {
  return {
...
function measurementSemantic(value: WorkflowCandidateMeasurementRecord): JsonObject {
...
function commandOutput(
  descriptor: WorkflowCommandDescriptor,
  result: WorkflowCommandExecutionResult,
): JsonValue {
...
async function admitAttempt(
  database: WorkflowRunDatabase,
  operation: WorkflowOperationRecord,
  effect: WorkflowAttemptRecord["effect"],
  now: () => Date,
...
async function registerVerification(
  database: WorkflowRunDatabase,
  value: Omit<WorkflowCandidateVerificationRecord, "runId">,
): Promise<WorkflowCandidateVerificationRecord> {

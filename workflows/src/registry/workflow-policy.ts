...
export const WORKFLOW_REGISTRY_POLICY_FILE = "registry.json";
export const WORKFLOW_REGISTRY_PROMOTION_FILE = ".registry-promotion.json";
 ...
 */
export async function readWorkflowRegistryPolicy(
  directoryInput: string,
  namespace: WorkflowNamespace,
  options: { ignorePendingPromotion?: boolean } = {},
): Promise<WorkflowRegistryPolicySnapshot> {
  const directory = path.resolve(directoryInput);
  const policyPath = path.join(directory, WORKFLOW_REGISTRY_POLICY_FILE);
  const transactionPath = path.join(directory, WORKFLOW_REGISTRY_PROMOTION_FILE);
...
export function defaultWorkflowRegistryPolicy(
  directoryInput: string,
  namespace: WorkflowNamespace,
): WorkflowRegistryPolicySnapshot {
  return policySnapshot(
    namespace,
    path.join(path.resolve(directoryInput), WORKFLOW_REGISTRY_POLICY_FILE),
    "default",
    [],
  );
}

export function workflowExposure(
...
export function workflowRegistryPolicyDocument(
  policy: WorkflowRegistryPolicySnapshot,
  name: string,
...
function policySnapshot(
  ...
  }) as JsonValue) as unknown as WorkflowRegistryPolicySnapshot;
}

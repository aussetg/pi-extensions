...
export class WorkflowReplayArtifactImporter {
  ...
  async importOperation(
    ...
    createdAt: string,
  ): Promise<WorkflowCallArtifactInput[]> {
    const result: WorkflowCallArtifactInput[] = [];
  ...
  private async importArtifact(
    sourceRecord: WorkflowArtifactRecord,
    createdAt: string,
    ...
    const proposed: WorkflowArtifactRecord = {
  ...
  private async validateSource(record: WorkflowArtifactRecord): Promise<void> {
    if (record.runId !== this.source.readRun().runId
      || record.bodyPath !== relativeBodyPath(record.digest)) {
      throw new WorkflowReplayArtifactError(`Source artifact ${record.digest} has invalid identity`);
    }
  ...
  private async validateTarget(record: WorkflowArtifactRecord): Promise<void> {
    if (record.bodyPath !== relativeBodyPath(record.digest)) {
      throw new WorkflowReplayArtifactError(`Target artifact ${record.digest} has a noncanonical body path`);
    }
...
async function validateBody(filePath: string, digest: string, bytes: number): Promise<void> {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size !== bytes) {
    throw new WorkflowReplayArtifactError(`Artifact body ${digest} is unsafe or has the wrong size`);
  }
  ...
  try {
    ...
    if (body.length !== bytes || sha256(body) !== digest) {
      throw new WorkflowReplayArtifactError(`Artifact body ${digest} failed its digest check`);
    }
  } finally {
    await handle.close();
  }
...
async function readOptionalMetadata(filePath: string): Promise<ArtifactMetadata | undefined> {
...
async function readMetadata(filePath: string): Promise<ArtifactMetadata> {
  const stat = await fs.promises.lstat(filePath);
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size > METADATA_BYTES) {
    throw new WorkflowReplayArtifactError("Artifact metadata file is unsafe");
  }
...
function assertSameArtifactContent(
  ...
  const content = (record: WorkflowArtifactRecord) => ({
  ...
  if (stableJson(content(left)) !== stableJson(content(right))) {
    throw new WorkflowReplayArtifactError(`Artifact ${right.digest} changed identity`);
  }
}
...
async function ensureRealDirectory(directory: string): Promise<void> {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()
    ...
    throw new WorkflowReplayArtifactError(`Unsafe artifact directory ${directory}`);
  }
}

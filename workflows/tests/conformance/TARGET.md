...
Concurrency and durable effects are available only through the public operations above.
...
- The run database persists immutable run/resource identity, sequential
...
- The production causal replay importer now validates one explicit source, reconstructs
...
- The production artifact layer stores canonical bodies/metadata, reconstructs exact
  ...
  unsafe segments, cycles, foreign identity, and filesystem tampering. An end-to-end wire test exercises a
...
- The projection path now derives review and inspector views from causal scope/operation
  ...
  four complete builtin renderer snapshots cover the projected native control structures.
- The production draft/tool path strictly reviews inert `.flow.ts`, binds promotion to source,
  ...
  and enumerate exact available measurement profiles. The coordinator is the only launcher.

...
lane ownership, revision conflicts, legacy-version refusal, and corruption detection.
...
The production artifact implementation now exists under `src/artifacts/*.ts`. Its schema-4 store
...
strictly checked by `typecheck:flows`. End-to-end schema-4 tests execute each definition through
...
The production review/projection path reads coherent schema-4 evidence into bounded plain
...
fields. The schema-4 coordinator is the only installed launcher.

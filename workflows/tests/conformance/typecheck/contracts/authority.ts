// Negative authority checks for the workflow runtime contract.
import {
  agent,
  command,
  schema as s,
...
export async function authorityErrors(
  ...
  // @ts-expect-error unavailable methods are not part of the flow contract
  await flow.unknownOperation("unsupported", async () => undefined);

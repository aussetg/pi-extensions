...
import {
  WorkflowCausalReplay,
  WorkflowCausalReplayError,
  type WorkflowCausalReplayFaultPoint,
} from "../src/runtime/causal-replay.js";
...
function testRoot(): string {
  const parent = path.resolve("node_modules/.workflow-tests");
...
function createRun(runDir: string, runId: string, workflowId: string, source = SIMPLE_SOURCE) {
  ...
  const parsed = source === SIMPLE_SOURCE
    ? SIMPLE_PARSED
    : parseWorkflow(source, { fileName: "/virtual/simple.flow.ts", apiPath: API });
  ...
  const ref: WorkflowDefinitionRef = {
    ...
    definitionHash: workflowDefinitionHash(workflowId as WorkflowDefinitionRef["id"], parsed),
    parsed,
  };
...
async function openReplay(
  ...
  crashAt?: WorkflowCausalReplayFaultPoint,
) {
  return track(await WorkflowCausalReplay.open({
...
function claim(
  ...
  scopeId: string,
  cursor: number,
  kind: WorkflowOperationRecord["kind"],
...
function completeFresh(
  ...
  input: {
    ...
    workspaceCheckpoint?: Parameters<WorkflowRunDatabase["completeCall"]>[0]["workspaceCheckpoint"];
    artifacts?: Parameters<WorkflowRunDatabase["completeCall"]>[0]["artifacts"];
  },
) {
  const previousCallKey = input.previous ?? WORKFLOW_ROOT_SCOPE_SEED;
  ...
  const scopes = database.createChildScopes(
    ...
    keys.map((laneKey) => ({
      ...
      seedKey: workflowLaneSeed({
        parentPreviousCallKey: WORKFLOW_ROOT_SCOPE_SEED,
...
function completeJoin(
  database: WorkflowRunDatabase,
  operation: WorkflowOperationRecord,
  scopes: Record<string, WorkflowScopeRecord>,
...
function completeScope(database: WorkflowRunDatabase, scope: WorkflowScopeRecord, terminalKey: string) {
...
async function createWorkspaceEffect(
  ...
  const bodySeed = workflowLaneSeed({
    parentPreviousCallKey: WORKFLOW_ROOT_SCOPE_SEED,
...
function time(database: WorkflowRunDatabase) { return iso(database.readRun().revision + 1); }

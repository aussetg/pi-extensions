...
import {
  WorkflowRecordedStructuralError,
...
describe("workflow v17 structured concurrency", () => {
  ...
  it("retains a sibling effect settlement that finishes while its lane is being cancelled", async () => {
    ...
    const outcome = await new WorkflowSemanticEngine(fixture.database, [adapter], {
      ...
      try {
        ...
        if (!(error instanceof WorkflowRecordedStructuralError)) throw error;
        caught = true;
      }
      return { caught };
    });
    ...
    for (let crashIndex = 0; crashIndex < points.length; crashIndex++) {
      ...
      await expect(new WorkflowSemanticEngine(fixture.database, [adapter], {
        ...
        faultInjector: (point, operation) => {
          if (transition++ === crashIndex) throw new WorkflowSemanticEngineCrashError(point, operation?.path);
        },
      }).run(successfulParallelProgram)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
  ...
  it("recovers failed and cancelled lanes at every fail-fast structural boundary", async () => {
    for (const target of [
      ...
      }).run(failFastProgram)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
  ...
  it("causally reuses an unchanged sibling while a changed lane and downstream call execute fresh", async () => {
    ...
    const adapter = new StructuredAdapter();
    const outcome = await new WorkflowSemanticEngine(target.database, [adapter], {
  ...
  it("executes failed structural joins fresh and ends downstream replay eligibility", async () => {
    ...
    const adapter = new StructuredAdapter({ failures: new Set(["bad"]) });
    expect(await new WorkflowSemanticEngine(target.database, [adapter], {
      now: clock(200), replay,
    }).run(failFastProgram)).toMatchObject({ status: "completed", result: { caught: true, after: 10 } });
    expect(adapter.executions.get("bad")).toBe(1);
    expect(adapter.executions.get("after")).toBe(1);
  ...
  it("rejects duplicate map keys and same-run structural policy drift before effects execute", async () => {
    ...
    await expect(new WorkflowSemanticEngine(drift.database, [adapter], {
      now: clock(),
      faultInjector: (point) => {
        if (point === "after-child-scopes-preclaimed") throw new WorkflowSemanticEngineCrashError(point);
      },
    }).run(successfulParallelProgram)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
    await expect(new WorkflowSemanticEngine(drift.database, [adapter], {
      now: clock(200),
...
class StructuredAdapter implements WorkflowSemanticEffectAdapter {
  ...
  async execute(context: WorkflowEffectAdapterContext): Promise<JsonValue> {
...
async function successfulParallelProgram(flow: WorkflowSequentialFlow): Promise<JsonObject> {
...
async function failFastProgram(flow: WorkflowSequentialFlow): Promise<JsonObject> {
  let caught = false;
  try {
    await flow.parallel({
      bad: async () => await call(flow, "bad", 0),
...
async function replayProgram(flow: WorkflowSequentialFlow, changed: number): Promise<JsonObject> {
...
async function mapReplayProgram(flow: WorkflowSequentialFlow, order: string[]): Promise<JsonObject> {
...
function createFixture(runId: string, concurrency = 4) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-structured-"));
  roots.push(root);
  ...
  const ref: WorkflowDefinitionRef = {
    ...
    definitionHash: workflowDefinitionHash("user:structured", PARSED),
    parsed: PARSED,
  };

...
import { WorkflowV17Registry, type WorkflowV17DefinitionRef } from "../src/registry/structured-workflows-v17.js";
...
import {
  ...
  type WorkflowV17AgentExecutionRequest,
...
interface BuiltinScript {
  agent(request: WorkflowV17AgentExecutionRequest): AgentReply;
  command?(request: Parameters<WorkflowV17CommandEffectExecutor["execute"]>[0]): JsonValue;
  asks?: JsonValue[];
...
async function createFixture(
  ...
  const authority = new WorkflowV17ControlAuthorityRegistry(`builtin:${name}:${database.readRun().runId}`);
  ...
  const metrics = ref.parsed.operations.some(value => value.method === "metrics")
    ? new WorkflowV17MetricSetRuntime(database, products, ref.parsed, now)
    : undefined;
  ...
  ) => void) => new WorkflowV17ExecutableRuntime({
    workflow: ref.parsed,
    invocation: snapshot,
    database,
    authority,
...
class BuiltinBackends {
  ...
  readonly agent: WorkflowV17AgentEffectExecutor = {
  ...
  readonly command: WorkflowV17CommandEffectExecutor = {
    execute: async request => {
      this.commandCalls++;
      return {
        ok: true,
  ...
  readonly ask: WorkflowV17AskExecutor = {
    ask: async () => {
      this.askCalls++;
  ...
  readonly verification: WorkflowV17VerificationExecutor = {
  ...
  readonly apply: WorkflowV17ApplyExecutor = {
    apply: async request => {
      this.applyCalls++;
...
class BuiltinCandidateDriver implements WorkflowV17CandidateWorkspaceDriver {
  private readonly changed = new Map<string, Set<string>>();

  constructor(
    private readonly store: WorkflowV17ArtifactStore,
    private readonly database: WorkflowRunDatabaseV17,
  ...
  write(workspace: WorkflowV17CandidateWorkspaceHandle, relativePath: string, contents: string): void {
...
class CandidateSensitiveMeasurementExecutor implements HostCommandExecutor {
  ...
  describe() {
    return { id: "workflow-v17-builtin-benchmark", protocolVersion: 1 as const, sandbox: "fake" as const };
  }
...
async function recoverOnce(
  ...
  predicate: (point: WorkflowV17SemanticEngineFaultPoint, operation?: WorkflowOperationV17Record) => boolean,
) {
  let crashed = false;
  try {
    ...
    if (!(error instanceof WorkflowV17SemanticEngineCrashError)) throw error;
  }
...
function staticResources(
  ...
  return workflowV17StaticEffectResources({
    workflow: ref.parsed,
    definitionHash,
    agents,
    commands,
...
function measurementProfile(): MeasurementProfileSnapshot {
  const definition = normalizeMeasurementProfile({
    ...
    argv: ["/usr/bin/workflow-v17-benchmark"],
    timeoutMs: 30_000,
    outputs: {
...
async function builtinRegistry(root: string): Promise<WorkflowV17Registry> {
...
function temporaryRoot(name: string): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `workflow-v17-builtin-${name}-`));
  roots.push(root);
  return root;
}

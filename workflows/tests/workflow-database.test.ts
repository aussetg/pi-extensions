...
} from "../src/runtime/causal-identity.js";
import type {
...
const SIMPLE_PARSED = parseWorkflow(SIMPLE_SOURCE, {
...
describe("workflow v17 run database", () => {
  it("creates and reopens one immutable-identity schema-4 WAL run", () => {
    ...
    expect(root).toMatchObject({
      ...
      kind: "root",
      siblingOrdinal: 0,
      seedKey: WORKFLOW_ROOT_SCOPE_SEED,
      status: "active",
    });
  ...
  it("serializes revisions across independent WAL connections", () => {
    const fixture = createDatabase();
    const contender = track(WorkflowRunDatabase.open(fixture.databasePath));
    ...
    expect(() => contender.transitionRun(1, { status: "paused", at: later(2) }))
      .toThrow(WorkflowRunDatabaseRevisionConflictError);
  ...
  it("uses scope-local cursors and restores caught failures into the causal chain", () => {
    ...
    const firstCallKey = causalCallKey(fixture.database, first.operation, {
      previousCallKey: WORKFLOW_ROOT_SCOPE_SEED,
  ...
  it("persists exact invocation-selected resources and detects post-launch corruption", () => {
    ...
    const ref: WorkflowDefinitionRef = {
      ...
      definitionHash: workflowDefinitionHash("user:optimize", OPTIMIZE_PARSED),
    ...
    raw.close();
    const corrupted = track(WorkflowRunDatabase.open(databasePath));
    expect(() => corrupted.validateIntegrity()).toThrow(WorkflowRunDatabaseCorruptionError);
  });
  ...
  it("detects cursor-chain and canonical-JSON corruption rather than repairing it", () => {
    ...
    completeRootEffect(fixture.database, rootScope, 0, "command", WORKFLOW_ROOT_SCOPE_SEED, "corrupt", { ok: true });
    fixture.database.close();
    open.delete(fixture.database);
    ...
    raw.close();
    const database = track(WorkflowRunDatabase.open(fixture.databasePath));
    expect(() => database.listEvents()).toThrow(WorkflowRunDatabaseCorruptionError);
    expect(() => database.validateIntegrity()).toThrow(WorkflowRunDatabaseCorruptionError);
  });
...
function createDatabase() {
  ...
  const ref: WorkflowDefinitionRef = {
    ...
    definitionHash: workflowDefinitionHash("user:simple", SIMPLE_PARSED),
    parsed: SIMPLE_PARSED,
  };
  const snapshot = createWorkflowInvocationSnapshot(ref, { value: "hello" }, {
    authority: "user",
    projectTrusted: false,
  });
...
function causalLaneSeed(
  ...
  spec: { childKind: Exclude<WorkflowScopeKind, "root">; laneKey?: string },
) {
...
function causalJoinKey(
  ...
  input: {
    ...
    policyHash: string;
    outputOrder: string[];
    lanes: Array<Pick<WorkflowStructuralJoinLaneRecord, "laneKey" | "terminalKey" | "outcome">>;
    result: JsonValue;
  },
) {
  return workflowStructuralJoinKey({
    ...input,
    operation: workflowOperationIdentity(operation),
  });
}
...
function storeArtifact(database: WorkflowRunDatabase, label: string) {
...
function temporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-database-"));
  roots.push(root);
  return root;
}

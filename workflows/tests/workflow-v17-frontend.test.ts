...
} from "../src/definition/workflow-v17-schema.js";
import { WORKFLOW_V17_RUNTIME_API_HASH } from "../src/definition/workflow-language-v17.js";
...
const parsedCorpus = new Map<string, ParsedWorkflowV17>();
...
describe("workflow v17 TypeScript frontend", () => {
  test.each(FILES)("parses and instruments the strict corpus: %s", (fileName) => {
    ...
    expect(parsed.transform.runtimeApiHash).toBe(WORKFLOW_V17_RUNTIME_API_HASH);
...
`, { fileName: "schema-probe.flow.ts" });
    ...
    expect(properties.path?.[WORKFLOW_V17_SAFE_PATH_KEY]).toBe(true);
    expect(properties.evaluator?.[WORKFLOW_V17_RESOURCE_KEY]).toBe("measurement-profile");
...
`, { fileName: "location-probe.flow.ts" });
  ...
  test("accepts native effectful loops and flags suspicious unbounded loops for review", () => {
    const parsed = parseWorkflowV17(workflowSource(`
    let index = 0;
...
`), { fileName: "helper.flow.ts" });
  ...
  test("accepts named parallel and candidate helpers with transitive contexts", () => {
    const parallel = parseWorkflowV17(workflowSource(`
...
`), { fileName: "parallel-helpers.flow.ts" });
    ...
    const candidate = parseWorkflowV17(workflowSource(`
...
`), { fileName: "candidate-helper.flow.ts" });
  ...
  test("applies candidate capture restrictions through helper chains", () => {
    expectWorkflowError(() => parseWorkflowV17(workflowSource(`
...
`), { fileName: "captured-container.flow.ts" }), /may not store captured state in mutable binding local/, {
  ...
  test.each([
    ...
    expectWorkflowError(
      () => parseWorkflowV17(workflowSource(body), { fileName: "invalid-helper.flow.ts" }),
      message,
      expected,
    );
  });
  ...
  test("rejects non-erasable TypeScript at its original location", () => {
    expectWorkflowError(() => parseWorkflowV17(`
import { schema as s, workflow } from "pi/workflows";
enum State { Ready }
const Output = s.object({ ok: s.boolean() });
export default workflow({
...
`;
    const left = parseWorkflowV17(source("Display A"), { fileName: "titles.flow.ts" });
    const right = parseWorkflowV17(source("Display B"), { fileName: "titles.flow.ts" });
...
function reviewProjection(parsed: ParsedWorkflowV17) {
  return {
...
function loadCorpus(fileName: typeof FILES[number]): ParsedWorkflowV17 {
  const cached = parsedCorpus.get(fileName);
  if (cached) return cached;
  const filePath = path.join(CORPUS, fileName);
  const parsed = parseWorkflowV17(fs.readFileSync(filePath, "utf8"), { fileName: filePath });

...
import {
  WorkflowSemanticDriftError,
  WorkflowSemanticEngine,
  WorkflowSemanticEngineCrashError,
  WorkflowRecordedEffectError,
  type WorkflowEffectAdapterContext,
  type WorkflowEffectIdentity,
  type WorkflowSemanticEffectAdapter,
...
describe("workflow cursor semantic engine", () => {
      ...
      const first = new WorkflowSemanticEngine(fixture.database, [adapter], {
        ...
        faultInjector: (point, operation) => {
          if (transition++ === crashIndex) {
            crashed = true;
            throw new WorkflowSemanticEngineCrashError(point, operation?.path);
          }
        },
      });
      await expect(first.run(loopProgram)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
  ...
  it("restores a durable failed settlement into ordinary catch control", async () => {
    ...
    const program = async (flow: WorkflowSequentialFlow) => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(WorkflowRecordedEffectError);
    ...
    await expect(first.run(program)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
  ...
  it("reopens SQLite after settlement and does not execute the physical effect twice", async () => {
    ...
    const first = new WorkflowSemanticEngine(fixture.database, [adapter], {
      now: clock(),
      faultInjector: (point) => {
        if (point === "after-effect-settled") throw new WorkflowSemanticEngineCrashError(point);
      },
    });
    ...
    const reopened = track(WorkflowRunDatabase.open(path.join(fixture.root, "run.sqlite")));
    const outcome = await new WorkflowSemanticEngine(reopened, [adapter], {
      now: clock(100),
  ...
  it("recovers an uncaught failure across every failed-effect boundary", async () => {
    for (const target of [
      ...
      }).run(program)).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
      expect(crashed).toBe(true);
      expect(await new WorkflowSemanticEngine(fixture.database, [adapter], {
        now: clock(100),
  ...
  it("rejects semantic input and effect-identity drift at the first cursor", async () => {
    ...
    await expect(new WorkflowSemanticEngine(inputFixture.database, [original], {
      now: clock(),
      faultInjector: (point) => {
        if (point === "after-operation-claim") throw new WorkflowSemanticEngineCrashError(point);
      },
  ...
  it("treats source sites and display titles as non-semantic during same-run restoration", async () => {
    ...
    await expect(new WorkflowSemanticEngine(fixture.database, [adapter], {
      now: clock(),
      faultInjector: (point) => {
        if (point === "after-operation-complete") throw new WorkflowSemanticEngineCrashError(point);
      },
    }).run(async (flow) => ({ total: (await call(flow, "same", 1, {
      sourceSite: "site-old", title: "Old title",
    })).value }))).rejects.toBeInstanceOf(WorkflowSemanticEngineCrashError);
  ...
  it("uses the causal importer before fresh execution", async () => {
    ...
    const outcome = await new WorkflowSemanticEngine(target.database, [targetAdapter], {
...
class CountingAdapter implements WorkflowSemanticEffectAdapter {
...
async function loopProgram(flow: WorkflowSequentialFlow) {
  let total = 0;
...
function createFixture(runId: string, maximumAgentLaunches = 100) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-engine-"));
  roots.push(root);
  ...
  const ref: WorkflowDefinitionRef = {
    ...
    definitionHash: workflowDefinitionHash("user:sequential", PARSED),
    parsed: PARSED,
  };
  const snapshot = createWorkflowInvocationSnapshot(ref, { value: "test" }, {
    authority: "user",

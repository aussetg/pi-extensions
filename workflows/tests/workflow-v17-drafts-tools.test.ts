...
import { WorkflowV17DraftService } from "../src/drafts/service-v17.js";
import { WorkflowDraftStore } from "../src/drafts/store.js";
import {
  projectWorkflowV17DraftPromotion,
  projectWorkflowV17DraftReview,
} from "../src/projection/approval-inspectors-v17.js";
import { WorkflowV17Registry } from "../src/registry/structured-workflows-v17.js";
import {
  readWorkflowV17RegistryPolicy,
  WORKFLOW_V17_REGISTRY_PROMOTION_FILE,
} from "../src/registry/workflow-v17-policy.js";
import {
  presentWorkflowV17InvocationSchema,
  workflowV17NamedToolParameters,
} from "../src/tool/named-workflow-v17.js";
...
import { registerWorkflowV17DraftTool } from "../src/tool/workflow-draft-v17.js";
...
describe("workflow v17 draft authoring", () => {
  it("strictly reviews inert .flow.ts drafts and exposes derived authority", async () => {
    ...
    expect(projectWorkflowV17DraftReview(review)).toMatchObject({
      runtimeVersion: 17,
      valid: true,
  ...
  it("binds promotion to source, review, policy, target exposure, and installed preimage", async () => {
    ...
    expect(projectWorkflowV17DraftPromotion(prepared.review, prepared.challenge)).toMatchObject({
      challenge: {
    ...
    expect(await readWorkflowV17RegistryPolicy(fixture.userTarget, "user")).toMatchObject({
      model: ["promote"],
    ...
    expect(await readWorkflowV17RegistryPolicy(fixture.userTarget, "user")).toMatchObject({ model: [] });
    await registry.refresh(fixture.project, {
      builtinDir: path.join(fixture.root, "missing-builtins"),
      userDir: fixture.userTarget,
      includeProject: false,
  ...
  it("fails registry discovery closed and exactly resumes a crashed two-file promotion", async () => {
    ...
    expect(fs.existsSync(path.join(fixture.userTarget, WORKFLOW_V17_REGISTRY_PROMOTION_FILE))).toBe(true);

    const blocked = new WorkflowV17Registry();
    ...
    expect(fs.existsSync(path.join(fixture.userTarget, WORKFLOW_V17_REGISTRY_PROMOTION_FILE))).toBe(false);
    const registry = new WorkflowV17Registry();
    await registry.refresh(fixture.project, {
      builtinDir: path.join(fixture.root, "missing-builtins"), userDir: fixture.userTarget,
      includeProject: false, apiPath: API,
...
describe("workflow v17 model tool presentation", () => {
  it("emits exact workflow branches and trust-filtered measurement-profile enums", async () => {
    ...
    const schema = workflowV17NamedToolParameters({
    ...
    const presented = presentWorkflowV17InvocationSchema(sourceSchema, [builtin, project]);
  ...
  it("stages only inert TypeScript source through the v17 draft tool", async () => {
    ...
    const ctx = context(fixture.project, true);
    let registered: any;
    registerWorkflowV17DraftTool({ registerTool: (tool: unknown) => { registered = tool; } } as any, fixture.service);
...
function setup(options: { promotionFault?: (point: "after-marker" | "after-source" | "after-policy" | "after-commit") => void } = {}) {
  ...
  const store = new WorkflowDraftStore({
    ...
    sourceExtension: ".flow.ts",
    ...(options.promotionFault ? { promotionFault: options.promotionFault } : {}),
  });
  const service = new WorkflowV17DraftService({ getThinkingLevel: () => "low" } as any, {
    store,
...
function temporaryRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-v17-phase15-"));
  roots.push(root);
  return root;
}

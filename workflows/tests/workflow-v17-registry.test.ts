...
} from "../src/registry/workflow-v17-policy.js";
import {
  createWorkflowV17InvocationSnapshot,
  readWorkflowV17InvocationSnapshot,
  workflowV17InvocationFilesystemPaths,
  writeWorkflowV17InvocationSnapshot,
} from "../src/persistence/workflow-v17-invocation.js";
import {
...
beforeEach(async () => {
  root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "workflow-v17-registry-"));
...
describe("workflow v17 filesystem registry", () => {
  test("discovers only .flow.ts definitions and applies fail-safe namespace policy", async () => {
    ...
    const registry = new WorkflowV17Registry();
  ...
  test("rejects unsafe files and malformed policy while keeping valid definitions human-only", async () => {
    ...
    await fs.promises.writeFile(
      path.join(userDir, WORKFLOW_V17_REGISTRY_POLICY_FILE),
  ...
  test("filename identity makes copy and rename explicit while exposure remains non-semantic", async () => {
    ...
    expect(workflowV17DefinitionHash(gammaModel.id, gammaModel.parsed)).toBe(gammaModel.definitionHash);
  ...
  test("policy snapshots have canonical semantic revisions", async () => {
    const absent = await readWorkflowV17RegistryPolicy(userDir, "user");
    await writePolicy(userDir, []);
    const explicit = await readWorkflowV17RegistryPolicy(userDir, "user");
    ...
    const sorted = await readWorkflowV17RegistryPolicy(userDir, "user");
...
describe("workflow v17 invocation snapshots", () => {
  test("binds launch actor and exact policy without admitting model launches of human workflows", async () => {
    ...
    expect(() => createWorkflowV17InvocationSnapshot(ref, { objective: "inspect" }, {
      authority: "model",
    })).toThrow(/human-only/);

    const snapshot = createWorkflowV17InvocationSnapshot(ref, { objective: "inspect" }, {
      authority: "user",
    });
    ...
    expect(() => createWorkflowV17InvocationSnapshot(ref, {}, { authority: "user" })).toThrow(/Invalid arguments/);
    ...
    expect(() => createWorkflowV17InvocationSnapshot(project, { objective: "inspect" }, {
      authority: "user",
  ...
  test("resolves and pins invocation-selected measurement authority with exact output roles", async () => {
    ...
    const snapshot = createWorkflowV17InvocationSnapshot(registry.resolve("user:optimize"), args, {
      authority: "model",
    ...
    expect(() => createWorkflowV17InvocationSnapshot(registry.resolve("user:optimize"), {
      ...args,
  ...
  test("writes and independently reconstructs canonical immutable snapshot files", async () => {
    ...
    const registry = new WorkflowV17Registry();
    await refresh(registry, false);
    const snapshot = createWorkflowV17InvocationSnapshot(
      registry.resolve("user:persisted"),
      { objective: "persist" },
      { authority: "model" },
    );
    const runDir = path.join(root, "run");
    await writeWorkflowV17InvocationSnapshot(runDir, snapshot);
    const paths = workflowV17InvocationFilesystemPaths(runDir);
    ...
    await writePolicy(userDir, []);
    const restored = await readWorkflowV17InvocationSnapshot(runDir);
    ...
    await expect(writeWorkflowV17InvocationSnapshot(runDir, snapshot)).rejects.toMatchObject({ code: "EEXIST" });
  }, 30_000);
  ...
  test("rejects snapshot source or transform tampering", async () => {
    ...
    await expect(readWorkflowV17InvocationSnapshot(runDir)).rejects.toThrow(/source transform is corrupt/);
  }, 30_000);
});
...
async function writePolicy(directory: string, model: string[]): Promise<void> {
  await fs.promises.writeFile(
    path.join(directory, WORKFLOW_V17_REGISTRY_POLICY_FILE),
    `${JSON.stringify({ formatVersion: 1, model })}\n`,
    "utf8",
  );
}

async function refresh(registry: WorkflowV17Registry, includeProject: boolean): Promise<void> {

...
describe("workflow v17 effect APIs and candidate lifecycle", () => {
  ...
  it("rejects missing static bindings and structural verification lookalikes", async () => {
    ...
    expect(() => workflowV17StaticEffectResources({
    ...
    expect(() => workflowV17VerificationRecord({
...
function createFixture(source: string, name: string) {
  ...
  const snapshot = createWorkflowV17InvocationSnapshot(ref, {}, { authority: "user", projectTrusted: false });
...
function runtime(
  ...
  options: { faultInjector?: (point: WorkflowV17SemanticEngineFaultPoint) => void } = {},
) {
  return new WorkflowV17ExecutableRuntime({
    workflow: fixture.parsed,
...
function scriptedBackends(
  ...
  const ask: WorkflowV17AskExecutor = {
    ask: async () => { calls.ask++; return { response: true, approvalId: "approval_ask" }; },
  };
  const command: WorkflowV17CommandEffectExecutor = {
  ...
  const agent: WorkflowV17AgentEffectExecutor = {
    execute: async request => {
      calls.agent++;
  ...
  const apply: WorkflowV17ApplyExecutor = {
    apply: async request => {
      calls.apply++;
      expect(request.workspace.record.state).toBe("frozen");
...
class FakeCandidateDriver implements WorkflowV17CandidateWorkspaceDriver {
  changed = false;

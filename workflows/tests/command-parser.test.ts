...
describe("parseWorkflowCommand", () => {
  ...
  it("rejects unknown, duplicate, and trailing command arguments", () => {
    ...
    expect(() => parseWorkflowCommand("open wr_1 result extra")).toThrow(/Too many arguments/);
    expect(() => parseWorkflowCommand("open wr_1 ui")).toThrow(/Open target/);
  });
  ...
  it("parses aliases and open target", () => {
    ...
    expect(parseWorkflowCommand("open wr_1 journal")).toEqual({ action: "open", runId: "wr_1", target: "journal" });
  });

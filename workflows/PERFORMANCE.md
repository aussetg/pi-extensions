...
- `/workflow` manager receives a precomputed run slice capped by `RENDER_LIMITS.managerRows`; previews are passive widgets and never take keyboard focus.
...
- Custom components cache rendered lines by width and local version state.

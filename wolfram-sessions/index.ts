import { CONFIG_DIR_NAME, getAgentDir, type ExecResult, type ExtensionAPI, type ExtensionContext } from "@earendil-works/pi-coding-agent";
...
function findWlSessionScript(ctx: ExtensionContext): string | undefined {
	const candidates = [
		...
		path.join(getAgentDir(), "skills", "wolfram-mathematica", "scripts", "wl-session"),
...
function configureEnvironment(ctx: ExtensionContext): string {
	const nextSessionBase = getSessionBase(ctx.sessionManager.getSessionId());
...
function parseCommandAction(args: string): CommandAction | undefined {
	...
	return COMMAND_ACTIONS.find((candidate) => candidate === action);
}
...
export default function wolframSessionsExtension(pi: ExtensionAPI) {
	pi.on("session_start", async (_event, ctx) => {
		const base = configureEnvironment(ctx);

		if (wlSessionScript) {
			await runWlSession(pi, base, ["gc"]);
		}
	});
	...
	pi.registerCommand("wolfram-sessions", {
		...
		handler: async (args, ctx) => {
			const base = sessionBase ?? configureEnvironment(ctx);

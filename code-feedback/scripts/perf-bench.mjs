...
function assertPiFeedback(result) {
  ...
  const found = content.some((part) => part?.type === "text" && typeof part.text === "string" && part.text.includes("code-feedback:"));
  if (!found) throw new Error("benchmark sanity check failed: expected inline code-feedback diagnostics");
}
...
function printHuman(output) {
  ...
  console.log(`code-feedback performance benchmark`);
...
function editScenarioNotes(options) {
  if (!options.enabled) return "same edit workload with code-feedback disabled";

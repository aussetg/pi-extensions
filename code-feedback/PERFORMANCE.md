...
Goal: measure whether `code-feedback` makes `pi` feel slower, then optimize only changes that are proven by repeatable numbers.
...
```bash
npm run perf
npm --silent run perf -- --json > /tmp/code-feedback-perf.json
npm run perf:live
```

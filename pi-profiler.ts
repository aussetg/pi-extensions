...
const WRAPPED = Symbol.for("pi-profiler.wrapped");
const GLOBAL_PATCH = Symbol.for("pi-profiler.map-set-patch");
...
function readConfig(pi: any): ProfilerConfig {
	...
	if (existsSync(CONFIG_PATH)) {
		try {
			...
			console.error(`[pi-profiler] Could not parse ${CONFIG_PATH}: ${formatError(error)}`);
		}
	}
...
export default function (pi: any) {
	...
	function withPatchedUi(ctx: any, source: string, fn: AnyFn, thisArg: any, args: any[]): any {
		...
		if (config.instrument.autocomplete && typeof ui.addAutocompleteProvider === "function") {
			...
			ui.addAutocompleteProvider = function profiledAddAutocompleteProvider(factory: AnyFn) {
				const wrappedFactory = function profiledAutocompleteFactory(current: any) {
		...
		if (config.instrument.uiRenderers && typeof ui.setFooter === "function") {
			...
			ui.setFooter = function profiledSetFooter(factory: any) {
				...
				const wrappedFactory = function profiledFooterFactory(...factoryArgs: any[]) {
		...
		if (config.instrument.uiRenderers && typeof ui.setWidget === "function") {
			...
			ui.setWidget = function profiledSetWidget(key: string, widget: any, options: any) {
				...
				const wrappedWidget = function profiledWidgetFactory(...widgetArgs: any[]) {
	...
	originalRegisterCommand?.("pi-profiler", {
		...
		handler: async (args: string, ctx: any) => {
			...
			if (action === "reset") {
				reset();
				ctx.ui?.notify?.("pi-profiler counters reset", "info");
				return;
			}
			...
			if (action === "jsc") {
				...
				ctx.ui?.notify?.(`pi-profiler JSC sampling for ${Number.isFinite(seconds) ? seconds : 10}s...`, "info");
				const paths = await runJscProfile(Number.isFinite(seconds) ? seconds : 10, Number.isFinite(intervalUs) ? intervalUs : 1000);
				ctx.ui?.notify?.(`pi-profiler JSC profile written: ${paths.textFile}`, "info");
				return;
			}

			if (action === "disable") {
				writeConfig({ ...config, enabled: false });
				ctx.ui?.notify?.(`pi-profiler disabled in ${CONFIG_PATH}; run /reload or restart pi`, "info");
				return;
			}

			if (action === "status") {
				ctx.ui?.notify?.(`pi-profiler enabled · ${stats.size} labels · report ${reportFile}`, "info");
				return;
			}

			const paths = writeSnapshot("command");
			ctx.ui?.notify?.(`pi-profiler report written: ${paths.reportFile}`, "info");
		},
	});

	originalOn?.("session_start", (_event: any, ctx: any) => {
		if (config.statusLine) ctx.ui?.setStatus?.("pi-profiler", "profile:on");
	});

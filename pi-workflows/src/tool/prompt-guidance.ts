export const WORKFLOW_TOOL_DESCRIPTION = "Run a deterministic JavaScript workflow that orchestrates multiple isolated Pi subagents. Use only for complex or non-standard orchestration that a single sequential agent cannot handle: explicit workflow requests, fan-out, adversarial verification, broad audits, large migrations, staged repeatable procedures, or telemetry dashboards. Do not use for work that can be done sequentially with normal tools or by one agent.";
...
export const WORKFLOW_PROMPT_GUIDELINES = [
  ...
  "Good workflow cases: explicit workflow requests, fan-out to multiple isolated subagents, adversarial cross-checking, broad repository audits, large migrations with independent phases, repeatable staged procedures, and declarative telemetry dashboards.",

...
export function projectRoot(cwd: string): string {
  ...
  while (true) {
    if (existsDir(path.join(current, ".git")) || existsDir(path.join(current, ".pi"))) return current;
...
export function projectWorkflowDir(cwd: string): string {
  return path.join(projectRoot(cwd), ".pi", "workflows");
}

export function workflowFilePath(scope: "user" | "project", cwd: string, name: string): string {
  const dir = scope === "project" ? projectWorkflowDir(cwd) : userWorkflowDir();
